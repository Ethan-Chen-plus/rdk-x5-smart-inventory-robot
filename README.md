...
two live RGB cameras + 7-joint state -> trained SmolVLA
    + item-specific Oreo/coffee language instruction
...
Prerequisites are Windows 11, Conda, Git, OpenSSH, ROKAE xCoreSDK
Python 0.7.0, and a configured RDK X5 Magic Box reachable by SSH.
The installer creates Python 3.12 and installs PyTorch 2.7.1 with its CUDA 12.8
wheel, the public Blackwell-compatible reproduction combination.
...
# Edit checkpoint, xcore_sdk_root, cameras, COM port, and item-specific tasks.
```
...
Inventory persists across launches. Use `-ResetDemo` only when intentionally
restoring the documented 7-coffee/5-Oreo starting scene.
...
both an empty-grasp rejection and a visually confirmed positive delivery. It
validates the RDK ROI/API gate independently; a physical rollout generated by
`run_policy.py` uses a UUID and writes the complete policy/gripper/API chain to
`outputs/rollouts/<uuid>.jsonl`.
